...
export default function Header() {
  ...
  return (
    <div className="px-6 md:px-12 py-3 bg-zinc-100/70 dark:bg-zinc-950/70 backdrop-filter backdrop-blur-md border-b sticky w-full top-0 flex justify-between items-center z-10">
      <Link href={"/"}>
      ...
      <div className="w-max hidden sm:block flex-[.3]">
        <Input
          ...
          onKeyDown={(e) => {
            ...
            else navigate.push(`/search/${e.currentTarget.value}`);
          }}
        />
      </div>
        ...
        <Popover>
          <PopoverTrigger>
            <article className="gap-3 items-center sm:py-1 sm:px-3 rounded-md flex hover:bg-zinc-50 dark:hover:bg-zinc-800">
              <Image
                className="rounded-full object-cover aspect-square w-8 sm:w-10"
                width={40}
                height={40}
              ...
              <div className="text-left hidden sm:inline-blockx">
                <p className="font-medium text-sm tracking-wide -mb-2 uppercase">
                  Jay Pokale
                </p>
          ...
          <PopoverContent
            className="border p-1 right-0 shadow-md mt-1 bg-white dark:bg-zinc-900 w-full flex-col flex rounded-lg"
            align="end"
          >
            <PopoverClose
